...
### Base URL
```
https://qtrade-apibacktunder.krkzfx.easypanel.host/api
```
...
### Exemplo de Login
```javascript
const response = await fetch('https://qtrade-apibacktunder.krkzfx.easypanel.host/api/auth/login', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    phone: '+5511999999999',
...
# Login
curl -X POST https://qtrade-apibacktunder.krkzfx.easypanel.host/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"phone": "11999888777", "password": "senha123"}'

# Gerar PIX (com token)
curl -X POST https://qtrade-apibacktunder.krkzfx.easypanel.host/api/pix/generate \
  -H "Content-Type: application/json" \
